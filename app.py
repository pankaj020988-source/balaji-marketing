...
st.markdown("""
    <style>
        ...
        /* 🖨️ ऑटो प्रिंट सिस्टीमचे आकर्षक बॅकग्राउंड */
        .print-box {
...
# ==========================================
# 💾 २. कायमस्वरूपी डेटा STORAGE मेमरी
# ==========================================
...
if "ad1_main_text" not in st.session_state: st.session_state.ad1_main_text = "🔥 **नोकर भर體 विशेष:** सर्व प्रकारचे ऑनलाईन जॉब फॉर्म्स, रेल्वे, पोलीस आणि बँक भरतीचे अर्ज अचूक भरून मिळतील."
...
# ==========================================
# 🗺️ ४. मुख्य स्क्रीनवर ३ कडक टॅब्स (तुमच्या गरजेनुसार)
# ==========================================
main_tab1, main_tab2, main_tab3 = st.tabs([
    "📢 चालू घडामोडी व जाहिराती", 
    "🌟 आमच्या प्रमुख सेवा", 
    "🖨️ मोबाईल ऑटो प्रिंट पोर्टल"
])

# ------------------------------------------
# टॅब १: डिजिटल नोटीस बोर्ड (जाहिराती)
# ------------------------------------------
with main_tab1:
...
# ------------------------------------------
# टॅब २: आमच्याकडील प्रमुख सेवा (डिजिटल कार्ड्स)
# ------------------------------------------
with main_tab2:
    ...
    with col_grid1:
        ...
        <div class="service-card" style="border-left-color: #e5be3b;">
            ...
            <div class="service-desc">सर्व प्रकारचे शासकीय दाखले, अधिकृत प्रपत्रे आणि सरकारी योजनांचे ऑनलाईन अर्ज अर्ज अचूकपणे भरून मिळतील.[cite: 1]</div>
        </div>
    ...
    with col_grid2:
        ...
        <div class="service-card" style="border-left-color: #20c997;">
            ...
            <div class="service-desc">केंद्र व राज्य शासनाच्या सर्व मेगाभरती, पोलीस, आर्मी, रेल्वे व विविध स्पर्धा परीक्षांचे नोकरभरती अर्ज १००% अचूक भरून मिळतील.[cite: 1]</div>
        </div>
        <div class="service-card" style="border-left-color: #007bff;">
            <div class="service-title">✈️ फ्लाईट बुकिंग केंद्र (Flight Tickets)</div>
            <div class="service-desc">देश-विदेशातील विमान प्रवासाचे तिकिट बुकिंग, हॉटेल्स आणि प्रवासाचे झटपट व खात्रीशीर आरक्षण सोल्यूशन्स.[cite: 1]</div>
        </div>
...
# ------------------------------------------
# 🖨️ टॅब ३: मोबाईल ऑटो प्रिंट पोर्टल (जसे image_e8d190.png मध्ये आहे)
# ------------------------------------------
with main_tab3:
    st.markdown("""
    <div style="text-align: center; margin-bottom: 15px;">
        <h2 style="color: #002f6c; margin-bottom: 2px;">👑 श्री बालाजी सायबर पॉईंट 👑</h2>
        <h4 style="color: #fd7e14; margin-top: 0;">स्मार्ट मोबाईल ऑटो प्रिंट सिस्टीम</h4>
    </div>
    """, unsafe_allow_html=True)
    
    # इमेज मधील फॉर्म रचना हुबेहूब तयार करणे
    st.markdown('<div class="print-box">', unsafe_allow_html=True)
    
    col_print_main, _ = st.columns([2, 1])
    with col_print_main:
        # १. प्रिंटचा प्रकार निवडा
        p_type = st.radio("🎨 प्रिंटचा प्रकार निवडा:", ["कलर (Color)", "ब्लॅक ॲंड व्हाईट (B&W)"], horizontal=True)
        
        # २. डॉक्युमेंटचा प्रकार
        d_type = st.radio("📄 डॉक्युमेंटचा प्रकार:", ["Normal (फोटो / PDF)", "आयडी कार्ड (Front & Back)"], horizontal=True)
        
        # ३. फाईल्स निवडा
        uploaded_print_files = st.file_uploader("📂 फाईल्स निवडा:", accept_multiple_files=True, key="auto_print_files")
        
        # ४. पेजेस सेटिंग (फक्त PDF साठी)
        ...
        st.markdown("<small style='color: #dc3545;'>*टीप: जर PDF मध्ये ९ पेजेस असतील, तर इथे ९ लिहा.</small>", unsafe_allow_html=True)
        
        # ५. पेज बाजू सेटिंग
        st.markdown("**🔄 पेज बाजू सेटिंग:**")
        both_side = st.checkbox("कागदाच्या दोन्ही बाजूंवर प्रिंट करा (Both Side)")
        
        # ६. पेमेंटची पद्धत निवडा
        pay_method = st.radio("💳 पेमेंटची पद्धत निवडा:", ["📱 ऑनलाईन (UPI/QR)", "💵 काउंटरवर रोख (Cash)"], horizontal=True)
        
        # एकूण रक्कम प्रदर्शन बॉक्स
        rate = 10 if "कलर" in p_type else 2
        total_amt = total_pgs * rate
        
        st.markdown(f"""
        <div style="background-color: #fff9db; padding: 15px; border-radius: 8px; text-align: center; border: 1px solid #ffe066; margin-top: 20px;">
            <h3 style="color: #f59f00; margin: 0;">💵 एकूण रक्कम: ₹{total_amt}</h3>
        </div>
        """, unsafe_allow_html=True)
        
        # सबमिट बटन
        if st.button("🚀 प्रिंट ऑर्डर सबमिट करा", type="primary", use_container_width=True):
            if uploaded_print_files:
                st.success("✅ तुमची प्रिंट ऑर्डर यशस्वीरित्या बालाजी प्रिंट सर्व्हरकडे पाठवली आहे!")
            else:
                st.warning("⚠️ कृपया आधी प्रिंट करायची फाईल अपलोड करा.")
                
    st.markdown('</div>', unsafe_allow_html=True)

st.write("")
st.markdown("""
<div style='background-color: #002f6c; color: white; padding: 15px; border-radius: 8px; text-align: center; font-size: 16px; font-weight: bold; border: 2px solid #d4af37;'>
    📍 पत्ताः बालाजी कॉम्प्लेक्स, माणगाव, रायगड, Maharashtra[cite: 1] | 📞 संपर्क: 8007365051[cite: 1] | 🟢 व्हॉट्सॲप: 8806789013[cite: 1]
</div>
""", unsafe_allow_html=True)
st.write("---")

# ==========================================
# ⚙️ ५. गुपित ओनर जाहिरात सेटिंग (फूटर्सच्या आत)
# ==========================================
col_foot1, col_foot2 = st.columns([5, 1])
with col_foot1:
    st.markdown("<p style='font-size: 13px; color: #555; margin-top: 10px; font-weight: bold;'>🙏 धन्यवाद ! पुन्हा भेट द्या![cite: 1] | Designed by Balaji Cyber Point</p>", unsafe_allow_html=True)
with col_foot2:
...
if show_admin:
    ...
    if secret_pass == st.session_state.owner_password:
        ...
        st.markdown("#### 🔑 ओनर पासवर्ड मॅनेजमेंट")
        ...
        with st.form("master_ads_form"):
            ...
            if submit_all:
                ...
                st.success("✅ सर्व बदल यशस्वीरित्या अपडेट झाले!")
                st.rerun()
    elif secret_pass != "": st.error("❌ चुकीचा पासवर्ड!")
